...
export type Containers<C> = IDictionary<React.ComponentType<any>, keyof C>;
export type ActionCreators<A> = IDictionary<ActionCreator<Action>, keyof A>;
export type Selectors<S> = IDictionary<(state: any, ...args: any[]) => any, keyof S>;

interface ICPart<C> { containers: C; }
interface IAPart<A> { actions: A; }
interface ISPart<S> { selectors: S; }

export type ResultEntry<C, A, S> = C & A & S & IReduxEntry;
