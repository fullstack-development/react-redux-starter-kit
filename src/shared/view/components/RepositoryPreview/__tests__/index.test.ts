...
import RepositoryAttribute, {
  IRepositoryAttributeProps,
} from '../RepositoryAttribute/RepositoryAttribute';
import {
  RepositoryPreview,
...
describe('(features/repositoriesSearch) RepositoryPreview component', () => {
  it('should call onOwnerClick with repository owner name on repository owner click', () => {
    ...
    const ownerAttribute: ShallowWrapper<
      IRepositoryAttributeProps
    > = component.find(RepositoryAttribute).find({ type: 'owner' });
